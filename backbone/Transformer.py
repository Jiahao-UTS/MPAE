...
class VisionTransformer(nn.Module):
    def __init__(self, point_num: int, input_resolution: int, patch_size: int, width: int,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, self.num_patches + point_num, width), requires_grad=True)

        scale = width ** -0.5
        self.class_embedding = nn.Parameter(scale * torch.randn(1, point_num, width))
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.cat([self.class_embedding.repeat(x.shape[0], 1, 1), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
...
def Get_vit(point_num: int, input_resolution: int, patch_size: int, width: int,
            layers: int, heads: int, output_dim: int):
    return VisionTransformer(point_num, input_resolution, patch_size, width, layers,
                             heads, output_dim)

if __name__ == '__main__':
    Vit = Get_vit(5, 128, 16, 256, 6, 8, 64)
