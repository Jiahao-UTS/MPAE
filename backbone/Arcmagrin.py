...
class ArcMarginProduct(nn.Module):
    ...
    def forward(self, input, global_F, distribution):
        # --------------------------- cos(theta) & phi(theta) ---------------------------
        # input [B, N, L]
        Bs = input.size(0)
        label = self.label.repeat(Bs, 1, 1).to(input.device)
        # cosine -> [B, N, N]
        global_F = F.normalize(global_F, dim=-1)
        input = F.normalize(input, dim=-1)
        cosine = torch.einsum("bln,bnc->blc", input, global_F.permute(0, 2, 1)) # 计算sine -> [B, N, N]
        ...
        mask = (distribution.sum(dim=(2, 3)) > 1e-3).float()
        loss = loss * mask
